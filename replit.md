...
- **Architecture**:
  - **Pure layer** (Task #1): `shared/export-columns.ts` (snapshot column registry + 5 bundles), `shared/export-historical.ts` (historical TIMESERIES + DAILY column registries, granularity enum, row estimator, 1-year/200k-row safety caps), `server/services/exports/{cell-builder,csv-serializer,excel-serializer,index}.ts` (`generateExport`), `server/services/exports/{historical-cell-builder,historical-generator}.ts` (historical writers).
...
- **Frontend UI**:
  - **Export dialog** (`client/src/components/ExportDialog.tsx`): top-level Mode toggle ("Fleet snapshot" / "Truck history"). Snapshot mode: bundle radio + format toggle + read-only filter chips + Advanced collapsible with grouped per-column checkboxes. Historical mode: truck single-select + range presets (24h/7d/30d/90d/1y/Custom with date inputs) + granularity radio (auto-suggested from range) + live `≈ N rows / ≈ M MB` estimate. Submit POSTs the appropriate body; 429 + 400 row-cap errors render inline. Accepts `initialMode`, `initialTruckId`, `initialRangeDays` props so callers can seed it.
  ...
  - **Truck detail integration** (`client/src/components/TruckDetail.tsx`): the "Export" button in the truck detail header opens the global ExportDialog seeded with `initialMode="historical"` + the truck's id + a 7-day range. Replaces the old in-place popover that hit the now-removed legacy endpoint.
