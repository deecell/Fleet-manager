...
- **Circuit-breaker / DB desync dead-zone (fixed)**: a stale `onConnect` from an in-flight `connect()` of a prior generation could fire *after* the breaker opened, clobbering the `flapping` DB row back to `online` while `isCircuitOpen` stayed true — stranding the device where neither the probe (only adopts `flapping`) nor the worker re-arm (skips `isCircuitOpen`) could reach it. Two-part fix: (1) `onConnect` bails immediately if `isCircuitOpen`, never writing `markDeviceConnected` or flipping status, so the row stays `flapping` and recovers normally; (2) a supervisor **backstop** (`_recoverStrandedDevices`, every 5 min) demotes any device with a non-recovery `connection_status` but stale `last_reported_at` (>20 min) back to `flapping` so the probe loop reclaims it. Freshness uses `last_reported_at` (per-poll), not `last_seen_at` (connect-only). For an *already-stranded* device, bounce only its cohort worker (`scripts/migrations/2026-06-25_unstick_*.sh`).
...
- **Reality check — InHand is NOT live GPS.** `location.source='cellTower'` fleet-wide (0/48 GPS), so the stored lat/lng is geolocated tower position, not a fix. A live drive test (GFR-69, 2026-06-25, ~45 mi) confirmed the location **never moved** over the whole drive, and the fresher cell/signal telemetry only refreshes **~every 5 min and is signal-gated** (router goes dark for 30+ min in weak coverage, parked or moving). Cell data is unusable for live tracking; a true route requires **GNSS enabled on the routers**. Details: `DEVELOPMENT_LOG.md` + `.agents/memory/inhand-location-is-celltower.md`.
