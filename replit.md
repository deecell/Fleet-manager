...
### Authentication
Both admin and customer authentication are session-based and use email/password with bcrypt. Platform admins are real `users` rows in the `Deecell Internal` org (slug `deecell-internal`) with `is_platform_admin = true`; the legacy shared `ADMIN_PASSWORD` is no longer accepted for login (it remains only as a fallback for `SESSION_SECRET` and to gate the one-shot migration endpoint). `platformAdminMiddleware` re-validates the user on every admin request — checking `isPlatformAdmin`, `isActive`, and the org's `isActive` flag — so revoking an admin takes effect immediately. Andy (`andy@deecell.com`) is seeded at startup with a NULL `password_hash` and uses the standard `/forgot-password` flow on first login. Additional admins are invited from `/admin/users` (Manage Platform Admins panel), which creates a user row with `is_platform_admin = true` and emails them an invitation token. `tenantMiddleware` ensures active user/organization verification for the customer surface.
...
The same async export pipeline is also used by admin pages, gated by `adminMiddleware` instead of `tenantMiddleware`. The first admin surface is `/admin/devices`: an "Export" button opens a dialog seeded with the page's current org/search filters and runs a cross-org device registry export through the same worker → S3 → email → sticky banner flow. To plug into the existing pipeline without forking it, a synthetic "Deecell Admin" user inside a "Deecell Internal" org is bootstrapped on first admin login and reused as `export_jobs.user_id` / `organization_id`, so concurrency limits and email lookup keep working. The `export_jobs` table grew a `kind` column (`'snapshot' | 'historical' | 'admin_devices'`) which the worker dispatches on; admin files land under `s3://…/exports/admin/<jobId>/<filename>`. The customer-facing dialog redesign is a follow-up; today only the snapshot UX is rendered to non-admins.
