...
export default function FleetTable({ trucks, selectedTruckId, onTruckSelect }: FleetTableProps) {
  ...
  return (
    <div className="flex gap-3 w-full">
      ...
      <div className="bg-white rounded-lg shadow-[0px_1px_3px_0px_rgba(96,108,128,0.09)] overflow-hidden flex-[26] mt-[21px]">
          ...
          <tbody>
            {sortedTrucks.map((truck, index) => (
              ...
              >
                ...
                <td className="px-3 py-3 text-[13px] 2xl:text-sm text-[#4a5565] whitespace-nowrap tabular-nums text-left">{truck.ah.toFixed(1)}</td>
                <td className="px-3 py-3 text-[13px] 2xl:text-sm font-medium text-black text-right whitespace-nowrap tabular-nums pl-[18px] pr-[18px]">{truck.temp.toFixed(1)}</td>
              </tr>
            ))}
          </tbody>
        </table>
