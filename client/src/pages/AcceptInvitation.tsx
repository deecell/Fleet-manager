...
import { Loader2, ArrowLeft, Lock, CheckCircle2, XCircle, Eye, EyeOff, UserPlus } from "lucide-react";
...
export default function AcceptInvitation() {
  ...
  const validateToken = async (tokenValue: string) => {
    try {
      const response = await fetch(`/api/auth/invitation/${tokenValue}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiRequest("POST", "/api/auth/accept-invitation", { token, password });
      setIsSuccess(true);
      toast({ 
          ...
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              <CardTitle className="text-2xl font-bold">
                {isSuccess ? "Account Activated!" : !isValid ? "Invalid Invitation" : "Create Your Password"}
              </CardTitle>
              <CardDescription className="mt-2">
                {isSuccess 
                  ...
                  : !isValid 
                    ...
                    : (
                      <>
                        Welcome{firstName ? `, ${firstName}` : ""}! You've been invited to join <strong>{organizationName}</strong>.
                        <br />
                        <span className="text-xs">Create a password to activate your account.</span>
                      </>
                    )
                }
              </CardDescription>
