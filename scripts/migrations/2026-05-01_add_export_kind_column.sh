#!/bin/bash
#
# Production Database Migration: Add `kind` column to export_jobs
# Created: 2026-05-01
#
# Adds the `kind` discriminator column introduced by Task #5 (Admin Devices
# Export Soft Launch). The export worker branches on this value to decide
# which generator + S3 path to use.
#
# Idempotent: uses ADD COLUMN IF NOT EXISTS, so safe to re-run. Existing
# rows backfill to 'snapshot' via the column default.
#
# Architecture:
#   - SQL lives in 2026-05-01_add_export_kind_column.sql (sidecar file).
...
#   ./scripts/migrations/2026-05-01_add_export_kind_column.sh
#
...
SQL_FILE="$SCRIPT_DIR/2026-05-01_add_export_kind_column.sql"
...
echo "Migration: Add export_jobs.kind column for admin export pipeline"
echo ""
...
COMMAND_ID=$(aws ssm send-command \
    ...
    --comment "Migration: add export_jobs.kind column" \
    --parameters "$PARAMS" \
