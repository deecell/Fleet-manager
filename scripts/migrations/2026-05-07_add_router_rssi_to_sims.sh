#!/bin/bash
#
# Production Database Migration: Add router_rssi + router_signal_updated_at columns to sims
# Created: 2026-05-07
#
# Adds two new columns to the sims table so the InHand poller can persist
# the cellular signal strength of each router on every poll cycle:
#   - router_rssi (integer, dBm — negative integer like -78)
#   - router_signal_updated_at (timestamp — bumped each poll, even when the
#     signal field is null, so freshness is observable)
#
# The /admin/devices page renders these as a "Router Sig" column alongside
# the existing PowerMon "PM Sig" column.
#
# Idempotent: ADD COLUMN IF NOT EXISTS, safe to re-run.
#
# Architecture:
#   - SQL lives in 2026-05-07_add_router_rssi_to_sims.sql (sidecar file).
...
#   ./scripts/migrations/2026-05-07_add_router_rssi_to_sims.sh
#
...
SQL_FILE="$SCRIPT_DIR/2026-05-07_add_router_rssi_to_sims.sql"
...
echo "Migration: Add sims.router_rssi + sims.router_signal_updated_at columns"
echo ""
...
COMMAND_ID=$(aws ssm send-command \
    ...
    --comment "Migration: add sims.router_rssi + sims.router_signal_updated_at columns" \
    --parameters "$PARAMS" \
