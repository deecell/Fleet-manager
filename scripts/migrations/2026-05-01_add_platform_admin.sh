#!/bin/bash
#
# Production Database Migration: Add users.is_platform_admin + Andy seed user (Task #8)
# Created: 2026-05-01
#
# Adds users.is_platform_admin and seeds the deecell-internal org +
# Andy (andy@deecell.com) as the first platform admin. Replaces the shared
# ADMIN_PASSWORD model with real per-admin email/password identity. Andy's
# password_hash is left NULL so first login goes through /forgot-password.
#
# Idempotent: ADD COLUMN IF NOT EXISTS + ON CONFLICT DO NOTHING + WHERE
...
echo "Migration: Add is_platform_admin + seed Andy as first platform admin"
echo ""
