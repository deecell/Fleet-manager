#!/bin/bash
#
# Production Database Migration: Add `truck_id` column to sim_location_history
# Created: 2026-05-05
#
# shared/schema.ts has defined `truck_id` on sim_location_history for a long
# time but it was never migrated to production. The historical export query
# filters by (organization_id, truck_id, recorded_at), so without this column
# every historical export crashes with `column "truck_id" does not exist`.
#
# Adds the column with ON DELETE SET NULL (matches the Drizzle schema),
# backfills truck_id from the owning SIM's current assignment, and creates a
# composite index (organization_id, truck_id, recorded_at) to keep the export
# query fast on busy orgs.
#
# Idempotent: ADD COLUMN IF NOT EXISTS / CREATE INDEX IF NOT EXISTS, safe to
# re-run.
#
# Architecture:
#   - SQL lives in 2026-05-05_add_sim_location_truck_id.sql (sidecar file).
...
#   ./scripts/migrations/2026-05-05_add_sim_location_truck_id.sh
#
...
SQL_FILE="$SCRIPT_DIR/2026-05-05_add_sim_location_truck_id.sql"
...
echo "Migration: Add sim_location_history.truck_id column + backfill + index"
echo ""
...
COMMAND_ID=$(aws ssm send-command \
    ...
    --comment "Migration: add sim_location_history.truck_id column + backfill" \
    --parameters "$PARAMS" \
