...
- **isCircuitOpen / DB-status desync dead-zone (FIXED 2026-06-25):** when the
  breaker opens it sets in-memory `isCircuitOpen=true` AND marks the row
  `flapping`, but a *racing connect-success DB write* could flip
  `connection_status` back to `online` (clobbering the flapping mark) without
  clearing `isCircuitOpen`. Result: DB says `online` so the supervisor solo-probe
  (only adopts `flapping`) ignored it, while the worker re-arm does
  `if (conn.isCircuitOpen) continue;` and skipped it too. The device sat
  `online`+`no_data` forever with NO log lines after the breaker-open instant — a
  true dead-zone. A DB nudge back to `flapping` does NOT fix an already-stranded
  device: the poisoned connection object is still in the worker's in-memory
  `connections` map, so `checkForNewDevicesInCohort` skips re-adoption
  (`if (this.connections.has(deviceId)) continue;`). For an *already-stranded*
  device, **bounce that cohort worker** (SIGTERM the child; supervisor
  auto-respawns) — surgical recovery script
  `scripts/migrations/2026-06-25_unstick_*.sh` (kills only the owning cohort
  workers, looked up live by `WORKER_COHORT_ID`).
  - **Durable lessons from the fix (impl detail in DEVELOPMENT_LOG.md):** the
    real culprit is a *stale* connect-success callback firing after the breaker
    opened — guard the success handler on `isCircuitOpen` so it can't clobber the
    `flapping` state; this is safe because the connect preamble already clears the
    breaker after cooldown. And any "stranded device" backstop must measure
    freshness by a *per-poll* timestamp (`last_reported_at`), never a
    *connect-only* one (`last_seen_at`), or long-lived healthy sessions
    false-positive.
