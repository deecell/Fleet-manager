...
**Root cause — 1-hour upload cap (2026-06-25):** the InHand router's *Device Manager* cloud service (Server `iot.inhandnetworks.com`, the feed our API reads) throttles **LBS info Upload Interval** (cell-tower location) and **Series Info Upload Interval** (signal/cell telemetry) to a **minimum of 1 hour** — the field accepts only integers 1–24 (hours; error "valid values are 1–24"). Devices are already at that minimum, so via the GUI you cannot push faster than hourly; sub-hour needs InHand (CLI/firmware) or a platform change, or switch to GNSS. **On top of the cap, LBS doesn't even honor its own 1-hour interval** (location stayed ~4.5 days stale), and the uploaded `cid`/`lac` aren't the live serving cell (unchanged across a 22-mi drive). So the stale InHand data is largely **config-capped + a server-side LBS staleness bug**, not a poller bug on our side. **Why:** confirmed by remote-login to the router config (Wireless Logic) + live API baselines.
